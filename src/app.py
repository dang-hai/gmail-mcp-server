...
from .vapi_handler import VapiGmailHandler
...
def voice_incoming():
    ...
    try:
        ...
        voice_handler = VoiceLLMHandler()
        
        twiml_response = voice_handler.process_voice_call(twilio_data)
...
def voice_process():
    ...
    try:
        # Get phone number and speech result from Twilio
        phone_number = request.form.get('phone') or request.args.get('phone')
        speech_result = request.form.get('SpeechResult', '')
        
        if not phone_number:
            # Try to extract from caller ID
            phone_number = request.form.get('From', '').replace('tel:', '')
        
        if not phone_number:
            raise Exception("Could not identify phone number")
        
        voice_handler = VoiceLLMHandler()
        twiml_response = voice_handler.process_speech_input(phone_number, speech_result)
        ...
        <?xml version="1.0" encoding="UTF-8"?>
        <Response>
            <Say>Sorry, I couldn't process what you said. Please try again.</Say>
        </Response>
        '''
        return response, 200, {'Content-Type': 'application/xml'}

@app.route('/vapi/functions', methods=['POST'])
def vapi_functions():
    ...
    try:
        ...
        # Add phone number to function call parameters
        message = webhook_data.get("message", {})
        if message.get("type") == "function-call":
            function_call = message.get("functionCall", {})
            if function_call.get("parameters") is None:
                function_call["parameters"] = {}
            function_call["parameters"]["phone_number"] = phone_number
        
        vapi_handler = VapiGmailHandler()
        result = vapi_handler.process_webhook(webhook_data)
        
        return jsonify(result)
        
    except Exception as e:
        print(f"Vapi function call error: {e}")
        return jsonify({
            "result": f"Sorry, there was an error processing your request: {str(e)}"
        }), 500

@app.route('/vapi/create-assistant', methods=['POST'])
def create_vapi_assistant():
    """Create a new Vapi assistant (for setup)"""
    try:
        vapi_handler = VapiGmailHandler()
        ...
        return jsonify({
            ...
            "message": "Gmail voice assistant created successfully"
        })
...
@app.route('/vapi/make-call', methods=['POST'])
def make_vapi_call():
    """Make an outbound call using Vapi (for testing)"""
    try:
        ...
        vapi_handler = VapiGmailHandler()
        call_result = vapi_handler.make_call(phone_number, assistant_id)
